...
async def y_new_d_task(call: CallbackQuery, state: FSMContext):
    ...
    await bot.send_message(
        chat_id=user_id,
        text=texts.THERE_ACCEPT_DELIVERY[lang].format(delivered_time, phone_line),
    )
...
@dp.callback_query_handler(lambda callbask_query: callbask_query.data.startswith("there_deliver:"), state="*")
